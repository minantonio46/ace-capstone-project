...
const PatientAuth = () => {
  ...
  const handleSubmit = () => {
    if (code.length === MAX_LENGTH) navigate('/dashboard');
    else alert("환자 코드 6자리를 모두 입력해주세요!");
  };
        ...
        <div className="w-[520px] bg-white rounded-[3.5rem] p-12 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.1)]">
          ...
          <div className="grid grid-cols-3 gap-5">
            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(n => (
              <button key={n} onClick={() => handlePress(n.toString())} className="h-20 rounded-2xl bg-[#f1f3f6] hover:bg-[#e4e7eb] text-3xl font-bold text-[#1a1c1e] transition-all active:scale-95">{n}</button>
            ))}
            <button onClick={handleBackspace} className="h-20 rounded-2xl flex items-center justify-center hover:bg-gray-50"><span className="material-symbols-outlined text-3xl text-gray-500 font-bold">backspace</span></button>
            <button onClick={() => handlePress("0")} className="h-20 rounded-2xl bg-[#f1f3f6] hover:bg-[#e4e7eb] text-3xl font-bold text-[#1a1c1e]">0</button>
            <button onClick={handleSubmit} className={`h-20 rounded-2xl flex items-center justify-center shadow-lg transition-all ${code.length === 6 ? 'bg-[#3b5998] text-white active:scale-95' : 'bg-gray-200 text-gray-400'}`}>
          ...
          <div className="mt-10 pt-8 border-t border-gray-50 flex justify-center">
            <button className="flex items-center gap-3 text-[#00478d] font-bold text-sm tracking-widest uppercase"><span className="material-symbols-outlined text-xl">barcode_scanner</span>환자 ID 팔찌 스캔</button>
          </div>
        </div>
      </div>
    </main>
